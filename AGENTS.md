# InovaTV API Intermediária — AGENTS.md

> **Como usar:** cole este arquivo no início de qualquer nova conversa
> (ou sessão do Codex) antes de pedir qualquer alteração neste
...
- **O Webhook do WhatsApp Cloud API / Meta** (`supabase/functions/webhook/`)
  — **aposentado/removido em 2026-09-07** (código retirado do
  repositório; a pasta pode existir vazia no disco, mas não representa
  arquitetura ativa). O canal de entrada WhatsApp atualmente ativo é o
  webhook-wasender, descrito a seguir.
...
conteúdo genuíno foi consolidado em `inovatv_central/AGENTS.md`) e
...
sincronização Rocket → Drive → Meta AI): `inovatv_central/AGENTS.md`,
